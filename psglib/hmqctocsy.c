#ifndef LINT
static char SCCSid[] = "@(#)hmqctocsy.c 1.2 10/11/94 Copyright (c) 1991-1994 Varian Assoc.,Inc. All Rights Reserved";
#endif
/* 
...
mleva()
{
 txphase(v11); xmtron(); delay(p1); xmtroff(); delay(getval("window"));
 txphase(v12); xmtron(); delay(2.0*p1); xmtroff(); delay(getval("window"));
 txphase(v11); xmtron(); delay(p1); xmtroff();
}

mlevb()
{
 txphase(v13); xmtron(); delay(p1); xmtroff(); delay(getval("window"));
 txphase(v2);  xmtron(); delay(2.0*p1); xmtroff(); delay(getval("window"));
 txphase(v13); xmtron(); delay(p1); xmtroff();
}

pulsesequence()
{
/* VARIABLE DECLARATION */
    double ss,arraydim,phase,j,bird,null;
    double cycles,trim,sstrim,mult,p1lvl,mix,window;
    int iphase;
