/*pge2.c - Pulse Field Gradient Diffusion experiment with 
           trapazoidal ramped gradients and a crusher 
           gradient during d5. 

           D. Rice 6/30/00

           Revised 10/02/00 with a crusher gradient. 
...
/* declare new variables*/
     ...
     double   dutycycle,p2,g1,g2,d0,d5,dac_cw,at,grad_cw,grad_p1,
              dac_p1,time,tramp,dac_hs,grad_hs,hsg1;
...
/*get new variables*/
     ...
     at=getval("at");
     tramp=getval("tramp");
     dac_hs=getval("dac_hs");
     grad_hs=getval("grad_hs");
     hsg1=getval("hsg1");
     getstr("stim",stim);
...
/*begin pulse sequence*/
     ...
     if(stim[0] == 'y') {
         ...
         if ((hsg1 > 0.0) && (hsg1 <= d5 - 10e-6)) {         
            delay(d5/2.0 - hsg1/2.0);
            dps_off();
            rampgrad(dac_hs,dac_cw,hsg1,tramp);
            dps_on();
            dps_show("zgradpulse",dac_hs,hsg1-tramp); 
            delay(d5/2.0 - hsg1/2.0);
         }
         else {
            delay(d5); 
