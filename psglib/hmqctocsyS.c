...
/* VARIABLE DECLARATION */
    double ss,arraydim,refoc,pwxs,ni,pwxlvl,phase,pwx,j,bird,satfrq,satdly;
    double cycles,trim,sstrim,mult,null,satpwr,p1lvl,pwxslvl,mix,nmix,window;
    int iphase;
    char satmode[MAXSTR],shape[MAXSTR];
...
/* LOAD VARIABLES */
    ...
    refoc = getval("refoc");  /* this stuff for selective experiment */
    pwxs = getval("pwxs");
...
/* nulling time for protons not coupled to 13C */
       ...
       if (pwxs != 0.0)
       rlpower(pwxslvl, DODEV);
       delay(d2/2.0);
       if (pwxs != 0.0)
	delay((pwxs/2.0) + 10.2e-6);
       rgpulse(2.0*pw,v1,0.0,0.0); rcvroff();
       delay(d2/2.0);
       if (pwxs != 0.0)
	{
		decshaped_pulse(shape,pwxs,v1,4.2e-6,rof1);
		delay(refoc);
	}
       else
	{
       ...
       txphase(v12);
	if (mix > 0.0)
       rlpower(p1lvl,TODEV); /* set power for mlev */ 
	if (dm[C] == 'y')
	rlpower(dpwr, DODEV);
       if (pwxs == 0.0)
       delay(bird - 4.2e-6);
