/* noesyjr -  2D cross relaxation experiment using jr "read" pulse
     ...
     (use "calfa" to set alfa for lp=0 to obtain flat baseline)
          ...
          G.Gray   Palo Alto  18 may 1992 (removed rcvroff/rcvron) 
*/
#include <standard.h>
pulsesequence()
{
   double phase,jrdelay,mix;
   char sspul[MAXSTR]; getstr("sspul",sspul);
   mix=getval("mix"); jrdelay=getval("jrdelay");
   loadtable("noesyjr45"); phase=getval("phase");
     initval(7.0,v2);
     stepsize(45.0,TODEV);
   getelem(t1,ct,v1);
   if (phase > 1.5) incr(v1);     /* hypercomplex phase shift */
   status(A);
     if (sspul[A]=='y')
      {
