/* hmqc11_3rf- heteronuclear multiple-quantum coherence with hypercomplex
         implementation, steady-state transients for either all or
         just the first t1 increment, pulse+receiver phasecycling during
         steady-state transients, proton pulses are 11echo pulses
   ...
   Parameters:
     ...
     pwx2lvl = power level for decoupler pulses
      ...
      tau =  1-1 echo delay
      sspul = 'y' for destroying all 1H magnetization before d1
...
/* BEGIN ACTUAL PULSE SEQUENCE CODE */
   status(A);
      ...
      rlpower(tpwr,TODEV);
     if (sspul[A] == 'y')                 /* kill all 1H magnetization */
      {
   ...
   status(B);
      ...
      delay(bird - 0.5*pw - 0.5*pwx2 - rof1);
         ...
         if (d2>0) delay(d2/2.0 - pw -rof1  - 2.0*pwx2/3.1414);
         decphase(v5);
	 rgpulse(pw, t4, rof1, 0.0);
         delay(2.0*tau);
