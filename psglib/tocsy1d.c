/* tocsy1d   -  1D tocsy for rapid scalar coupling correlations;
           ...
           SELECTIVE PULSE IS DONE VIA LINEAR MODULATORS
    ...
    nt - multiple of 32  (intsub = 'n')
         multiple of 64  (intsub = 'y')
            ...
            S.Cheatham changed for UNITYplus / apshaped_pulse / 5.1, Oct 1995  */
...
pulsesequence()
{
  ...
  status(B);
    ...
    if (seltime>0)
    {
      ...
      apshaped_pulse(shape,seltime,zero,t11,t12,rof1,rof2);
      offset(tof,TODEV);
    }
    rlpower(p1lvl,TODEV); 
