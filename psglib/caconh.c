/* ca(co)nh.c     Grzesiek and Bax (J. Biol Molecular NMR 185,204)
                    (modified to give ca correlations only)

                  For mid-sized proteins (15-20KDa) this sequence is 2 fold more
                  sensitive than the NH(CO)CA
   ...
   pwcolvlh = power level for hard C0 pulses (dof=cofrq)
      ...
      cofrq = freq of co for hard pulses
       padj = phase 7 on co pulse (set to about 330)
   ...
   amidefrq = amide frequency
     ...
     gzlvl5 = purge gradient before detection
       grtn = gradient recovery time for nth gradient

nt=16, minimum is 8
adjust signal for max using padj

   Gordon Rule, Univ. Virginia
*/
...
/* VARIABLE DECLARATION */
   ...
   double       ss,epsilon,kappa,lambda,eta,tndelay,delta,
		...
		hd90, hdlvl, hdres,
                ...
                gt1,gt2,gt3,gt4,gt5,grt1,grt2,grt3,grt4,grt5;
...
/* LOAD VARIABLES */
   ...
   zap = getval("zap");
   gzlvl1 = getval("gzlvl1");   gzlvl2 = getval("gzlvl2");
   gzlvl3 = getval("gzlvl3");   gzlvl4 = getval("gzlvl4");
   gzlvl5 = getval("gzlvl5");
      gt1 = getval("gt1");      gt2 = getval("gt2");
      gt3 = getval("gt3");      gt4 = getval("gt4");
      gt5 = getval("gt5");
   getstr("coshape", coshape);
   getstr("hdshape", hdshape);
   getstr("sspul", sspul);
   getstr("fad1", fad1);    getstr("fad2", fad2);
...
/* LOAD PHASE TABLE */
   ...
   getelem(t8,ct,v8);
   getelem(t10,ct,oph);
/*Calculate Gradient Recovery Times*/
   grt1=fabs(gzlvl1*2e-8);   if (grt1< 50e-6) grt1=50e-6;
   grt2=fabs(gzlvl2*2e-8);   if (grt2< 50e-6) grt2=50e-6;
   grt3=fabs(gzlvl3*2e-8);   if (grt3< 50e-6) grt3=50e-6;
   grt4=fabs(gzlvl4*2e-8);   if (grt4< 50e-6) grt4=50e-6;
   grt5=fabs(gzlvl5*2e-8);   if (grt5< 50e-6) grt5=50e-6;
/* CHECK CONDITIONS */
   ...
   if( getval("ni")*(0.5/sw1)+pwco > zeta)
   {
      printf("carbon sweep width is too wide for pol transfer\n");
   }
   if( zap > .0015)
   {
...
/* INEPT transfer from H1 to C13 */
      ...
      delay(delta - (pwca/2.0) - 2*rof1);
      simpulse(2*pw,pwca, zero, t2, rof1, rof1);
...
/*Constant time t1 evolution of ca */
      decrgpulse(pwca, v1, rof1, rof1);
      rlpower(pwcolvl,DODEV);	
      if ( (tau1/2.0) > (epsilon + (2.0*pwn15)) )
      {
      delay(epsilon);
      obsprgon(hdshape, hd90, hdres);
      xmtron();
      delay( (tau1/2.0)-epsilon-pwn15);
      dec2rgpulse(2*pwn15,zero,rof1,rof1);
      delay(zeta - pwn15 - pwco);
      }
      else
      {
      delay( (tau1/2.0) - pwn15 );
      dec2rgpulse(2*pwn15,zero,rof1,rof1);
      delay( epsilon - (tau1/2.0) - pwn15);
      obsprgon(hdshape, hd90, hdres);
      xmtron();
      delay ( zeta + (tau1/2.0) - epsilon - pwco );
      }
      decshaped_pulse(coshape,pwco,zero,rof1,0.0);
      rlpower(pwcalvl+6,DODEV);
      decrgpulse(pwca,t5,0.0,rof1);
      rlpower(pwcolvl,DODEV);
      delay( zeta-(tau1/2.0) - pwco - pwca);
      decshaped_pulse(coshape,pwco,zero,rof1,0.0);
      rlpower(pwcalvl,DODEV);
