...
REF: B. Rief, M. Koeck, R. Kerssebaum, H. Kang,
    ...
    recovery- gradient recovery delay
...
INOVA implementation: Norbert Zimmermann, BASF AG 08.08.96
      ...
      modified : Peter Sandor, Datmstadt, 2002 - all C13 180s are shaped 
*/
...
/* construct a command line for Pbox */
   ...
   { sprintf(comm, "Pbox ad180 -w \"cawurst-10 %.1f/%.6f\" -s 1.0 -0\n",
     ...
     sprintf(comm, "Pbox av180 -w \"av180b %.6f\" -s 1.0 -0\n", av180);
   ...
   if (kappa == 1.0)
      {
        ...
        if (maxni < ni) 
          {printf("too many increments! maxni = %d   ",maxni); abort(1); }
      }
...
/* BEGIN ACTUAL PULSE SEQUENCE */
 ...
 status(B);
   ...
   delay(tau1);
   delay(gt1 + recovery + 2*GRADIENT_DELAY - 2*pw -1.0e-6);
   ...
   delay(recovery); 
   delay(taucc - 10.0e-6 - recovery - gt2 - 2*GRADIENT_DELAY - 1.0e-6);
   ...
   delay(gt3+recovery+2*GRADIENT_DELAY-POWER_DELAY-1.0e-6);
   rgpulse(2*pw,t1,1.0e-6,rof2);
